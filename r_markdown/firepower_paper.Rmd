...
```{r}
  ...
  field_plot_3 <-
    ...
    dplyr::filter(grepl("co|pm|ec", pol), grepl("mean", var), grepl("trad|justa|eco_h|estufa", stove)) %>%
  ...
  field_plot_4 <-
    rose_data %>%
    dplyr::filter(grepl("rocket|justa|traditional|mud|metal|ceramic charcoal [:(:]", stove)) %>%
...
```{r}
  ggplot(field_plot, aes(y = bc_ef, x = protocol)) +
    geom_boxplot(outlier.shape = NA) +
    geom_jitter(aes(color = fuel), width = 0.15, shape = 4, stroke = 1) +
    theme_bw() + 
    scale_y_log10() +
