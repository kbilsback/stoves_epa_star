...
```{r}
  jetter_plot <-
    ...
    dplyr::left_join(jetter_data %>%
                     ...
                     dplyr::select(-stdev, -pol),
                     by = c("protocol", "stove", "fuel", "fuel_notes", "protocol_notes", "id")) %>%
    dplyr::left_join(jetter_data %>%
...
```{r fig_2, fig.height=8, fig.width=10}
  grav %>%
    ...
    ggplot(aes(x = fp, y = mce)) +
      ...
      geom_errorbar(data = jetter_plot, color = "black",
                    aes(x = firepower, y = mce, ymin = mce_min, ymax = mce_max), width = 0.5) +
      geom_point(data = jetter_plot,
                 aes(x = firepower, y = mce), color = "black",
                 size = 4) +
      geom_point(data = jetter_plot, aes(x = firepower, y = mce,
                                         colour = fuel), size = 2) +
      theme_bw() + 
...
```{r SI_XX_1, fig.height=8, , fig.width=10}
  grav %>%
    dplyr::filter(grepl("fp|mce", var), !grepl("ceramic|Philips|small metal|elbow [:(:]Envirofit|three", stove)) %>%
    dplyr::select(-units) %>%
    tidyr::spread(var, val) %>%
    ggplot(aes(x = fp, y = mce)) +
      geom_point(size = 3, aes(color = fuel), shape = 1, stroke = 1.5) +
...
```{r, eval=FALSE}
  field_plot <-
    ...
    dplyr::filter(grepl("co_ef|pm_ef|bc_ef", var),
                  grepl("plancha|rocket|clay ring|small", stove)) %>%
    ...
    tidyr::spread(var, val) %>%
    dplyr::select(stove, fuel, co_ef, pm_ef, bc_ef, sample_id, test_type) %>%
  ...
  field_plot_2 <-
    ...
    dplyr::filter(grepl("co|pm", pol),
                  grepl("mean", var),
                  grepl("rocket|plancha|greenfire|charcoal [:(:]small[:):]", stove)) %>%
    dplyr::mutate(stove = gsub("rocket .*", "rocket elbow", stove)) %>%
...
```{r fig_4_1, fig.height=8, , fig.width=10, eval=FALSE}
...
```{r, fig.height=4, fig.width=13, eval=FALSE}
  field_plot %>%
