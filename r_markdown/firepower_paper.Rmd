...
```{r}
  jetter_plot <-
    ...
    dplyr::filter(grepl("three|gasifier|justa|charcoal [:(:]|rocket", stove),
                  fuel_notes == "dry") %>%
    dplyr::mutate(stove = gsub(" [0-9]", "", stove))
```
...
```{r fig_2, fig.height=8, , fig.width=10}
  grav %>%
    dplyr::filter(grepl("fp|mce", var), grepl("ceramic|Philips|justa [:(:]art|small metal|elbow [:(:]Envirofit|three", stove)) %>%
    dplyr::select(-units) %>%
...
```{r, eval=FALSE}
  field_plot <-
    ...
    dplyr::filter(grepl("co_ef|pm_ef|bc_ef", var),
                  grepl("three|justa|rocket|clay ring|small", stove)) %>%
