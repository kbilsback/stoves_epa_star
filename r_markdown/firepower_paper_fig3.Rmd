...
```{r, fig.width=4.5, fig.height=3}
  ...
  ggplot(wbt_cont) +
  #geom_errorbarh(aes(x = pm_mean, y = co_mean, xmax = pm_max, xmin = pm_min), height = 0, size = 1) +
  #geom_errorbar(data = wbt_cont,
  #              aes(x = pm_mean, y = co_mean, ymax = co_max, ymin = co_min), width = 0, size = 1) +
  geom_point(data = wbt_cont, aes(x = pm_mean, y = co_mean, color = fuel, size = fp_mean), alpha = 0.5) +
  geom_point(data = mass_ef %>%
               dplyr::filter(grepl("three|rocket|ceramic plancha", stove), grepl("continuous", fst_type)),
             aes(x = pm_ef, y = co_ef, color = fuel, size = fp), alpha = 0.5) +
  theme_bw() +
  scale_x_log10() + 
  scale_y_log10() + 
  ...
  labs(fill = "", color = "Fuel", size = "Firepower (kW)")
```
...
```{r, fig.width=4.5, fig.height=3}
  ...
  #geom_errorbar(data = wbt_batch,
  #              aes(x = pm_mean, y = co_mean, ymax = co_max, ymin = co_min), width = 0, size = 1) +
  geom_point(data = wbt_batch, aes(x = pm_mean, y = co_mean, color = fuel, size = fp_mean), alpha = 0.5) +
  geom_point(data = mass_ef %>%
                    dplyr::filter(grepl("metal jiko|ceramic jiko|forced", stove), grepl("batch", fst_type)),
             aes(x = pm_ef, y = co_ef, color = fuel, size = fp), alpha = 0.5) +
  scale_color_manual(values = c("#F0E442","#999999", "#0072B2", "#D55E00", "#CC79A7")) + 
  scale_x_log10() + 
  scale_y_log10() + 
  theme_bw() +
  theme(text = element_text(size = 14),
