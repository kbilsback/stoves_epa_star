...
```{r fig_3a, fig.width=9, fig.height=3}
  wbt_cont <-
    wbt_plot %>%
    dplyr::filter(grepl("continuous", fst_type))
  ...
  ggplot(data = mass_ef %>%
    ...
    geom_point(aes(x = pm_ef, y = co_ef, color = fuel, size = fp), alpha = 0.5) +
    geom_point(data = wbt_cont, aes(x = pm_mean, y = co_mean, color = fuel, size = fp_mean)) +
    theme_bw() +
    scale_x_log10() + 
    scale_y_log10() + 
    scale_color_manual(values = cbPalette,
                       guide = guide_legend(override.aes = list(size = 3, alpha = 0.5))) + 
    theme(text = element_text(size = 14),
            plot.title = element_text(hjust = 0.5),
...
```{r fig_3b, fig.width=9, fig.height=3}
  wbt_batch <-
    wbt_plot %>%
    dplyr::filter(grepl("batch", fst_type))
  ...
  scale_color_manual(values = c("#0072B2", "#999999", "#CC79A7", "#009E73", "#56B4E9"),
                      guide = guide_legend(override.aes = list(size = 3, alpha = 0.5))) + 
  scale_x_log10() + 
  scale_y_log10() + 
  theme_bw() +
...
```{r fig_3c, fig.width=9, fig.height=3}
  ggplot(data = mass_ef %>%
