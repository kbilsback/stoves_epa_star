...
```{r}
  temp_merged_all <-
    temp_merged_all %>%
    dplyr::filter(!(hh_id == "IN7" & date == "2016-04-20" & time < 43200),
...
```{r plot_test_temp, fig.width=2, fig.height=4}
...
```{r long_data_conversion}
  temp_merged_all <- dplyr::select(temp_merged_all, hh_id, date, time, datetime, temp) %>%
                     dplyr::mutate(field_site = "india")

  temp_merged <- dplyr::select(temp_merged, hh_id, date, time, datetime, temp) %>%
                     dplyr::mutate(field_site = "india")
```
