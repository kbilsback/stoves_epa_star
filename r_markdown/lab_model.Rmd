...
# Build basic linear firepower model
  ...
  basic_rmse_kw <- mean(rmse$rmse_kw)
  summary(basic_fp)
  plot(basic_fp)
```

The resulting linear model is: $$\text{BC rate} = `r round(basic_coef[2], digits = 3)` \times FP `r round(basic_coef[1], digits = 3)`$$ with an *rmse* of `r round(basic_rmse_kw, digits = 1)` kilowatts. We can test the predictive ability of the model using leave-one-out cross validation.
