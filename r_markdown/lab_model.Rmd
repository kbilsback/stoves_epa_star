...
The resulting linear model is: $$\text{BC rate} = `r round(basic_coef[2], digits = 3)` \times FP `r round(basic_coef[1], digits = 3)` + \times MCE `r round(basic_coef[3], digits = 3)`$$ with an *rmse* of `r round(basic_rmse, digits = 1)` miligrams per minute [note: each stove/fuel combination is weighted equally in rmse calculation]. We can test the predictive ability of the model using leave-one-out cross validation.
