...
```{r}
  samples <-
    ...
    dplyr::mutate(stove = tolower(stove),
          ...
          dplyr::mutate(stove =
                          stove %>%
                          forcats::fct_recode("imp. plancha (chimney)" = "justa (hm5000)",
                                              "trad. plancha (chimney)" = "justa (artisan)",
                                              ...
                                              "imp. metal jiko" = "metal jiko (burn)",
...
```{r}
  field_data <- 
    field_data %>%
    dplyr::mutate(stove = gsub(".*estufa.*", "imp. plancha (chimney)", stove),
                  stove = gsub(".*eco_h.*", "imp. plancha (chimney)", stove),
                  stove = gsub(".*justa.*", "trad. plancha (chimney)", stove),
...
```{r}
  rose_data <-
    ...
    dplyr::mutate(stove = ifelse(grepl("ceramic|UGA", stove), "ceramic jiko", stove),
                  stove = gsub(".*justa.*", "imp. plancha (chimney)", stove),
                  stove = gsub(".*traditional.*", "trad. plancha (chimney)", stove),
                  fuel = "wood (collected)") %>%
