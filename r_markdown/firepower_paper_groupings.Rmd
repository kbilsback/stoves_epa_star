...
```{r}
  field_data <- 
    ...
    dplyr::mutate(stove = gsub(".*estufa.*", "metal plancha", stove),
                  ...
                  fuel = gsub(".charcoal.*", "hardwood (lump)", fuel),
...
```{r}
  field_data_2 <-
    rose_data %>%
    dplyr::mutate(stove = ifelse(grepl("ceramic rocket", stove), "rocket elbow", stove),
                  stove = ifelse(grepl("ceramic|UGA", stove), "ceramic jiko", stove),
                  stove = gsub(".*justa.*", "ceramic plancha", stove),
                  stove = gsub("mud chula", "mud chulha", stove),
                  stove = gsub(".*traditional.*", "ceramic plancha", stove),
                  fuel = gsub(".*hardwood.*", "hardwood (lump)", fuel),
                  fuel = gsub(".*wood.*", "wood (collected)", fuel),
                  fuel = ifelse(field_site == "uganda", "hardwood (lump)", fuel)) %>%
    dplyr::mutate(test_type = "Field") %>%
    dplyr::select(-notes)
```
...
```{r}
  ...
  saveRDS(field_data_2, "../r_files/field_data_2.RDS")
