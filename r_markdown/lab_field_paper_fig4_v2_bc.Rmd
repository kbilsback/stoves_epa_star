...
```{r, echo=FALSE}
  outcomes <-
    lab_data %>%
    dplyr::filter(id != "3B", fuel != "okote", grepl("pm_ef|bc_rate|co_rate", var)) %>%
...
```{r}
  mod1 = lm(log(pm_ef) ~ stove, data = mod_data)
  mod2 = lm(log(pm_ef) ~ fp*stove, data = mod_data)
  mod3 = lm(log(pm_ef) ~ mce*stove + fp*stove, data = mod_data)
...
```{r}
  pm_est_stove <-
    pm_est %>%
    dplyr::left_join(dplyr::select(field_rates, id, stove, pm_ef), by = c("id")) %>%
