...
```{r tidy_time_var}
 # sample durations (minutes)
  field_test_times <- dplyr::filter(field_test_times, var == "sample_start" | var == "sample_end") %>%
...
```{r test_grid, fig.width=12, fig.height=6}
...
```{r test_time_hist, fig.width=12, fig.height=8}
  p_hist <- ggplot(field_test_times, aes(x = dur)) +
            geom_histogram(binwidth = 15) +
            theme_minimal() +
            xlab("test duration (min)")

  p_data <- dplyr::mutate(field_test_times,
                          value_norm = (dur - mean(dur, na.rm = TRUE)) / sd(dur, na.rm = TRUE),
                          outlier = ifelse(is_outlier(dur),
                          as.character(id),
                          NA))

  p_box <- ggplot(field_test_times, aes(x = "duration", y = value_norm)) +
           geom_boxplot() +
...
```{r flow_hist}
  ...
  p_hist <- ggplot(p_times, aes(x = val)) +
            geom_histogram(binwidth = 15) +
            facet
            theme_minimal() +
            xlab("test duration (min)")
