...
```{r grav_notes}
  notes_g1 <- dplyr::filter(filter_meta, grepl("Uncontrolled Test|Blank", filter_type)) %>%
              ...
              dplyr::filter(!is.na(notes)) %>%
              dplyr::mutate(inst = ifelse(grepl("^[A-Z][A-Z]$",cart_id), "grav:ecoc", "ecoc")) %>%
              dplyr::select(-cart_id)

  notes_g2 <- dplyr::select(grav, id, notes) %>%
              dplyr::filter(grepl("^IN[0-9]",id)) %>%  # only deals with emissions tests
              dplyr::left_join(dplyr::select(filter_meta, hh_id, filter_type, teflon_id), by = c("id" = "teflon_id")) %>%
              dplyr::filter(notes != "") %>%
...
```{r adjust_flags}
  notes <- dplyr::mutate(notes, qc = ifelse(grepl("Filter damaged", notes), "bad", as.character(qc)))
```
