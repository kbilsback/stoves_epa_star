...
# Load data.  time and date variables are for the analysis date.  Need to combine with grav or other data stream to get the correct sample info based on the sampleid or hhid.  Currently the sampleid in the ecoc data stream doesn't match with other data streams.
...
```{r sample_merge}
  ...
  ecoc_merged_all <- dplyr::mutate(ecoc, date = replace(date, date == "2016-04-25", "2016-04-26")) %>%
...
```{r fix_datetime}
  ecoc_merged_all <- dplyr::arrange(ecoc_merged_all, datetime) %>%
                    dplyr::distinct() %>%
...
* add units
```{r add_units}
  ecoc_merged_all <- dplyr::mutate(ecoc_merged_all, units = "ppm")
```
...
```{r merge_flags}
  # fix warning
  ecoc_merged_all <- dplyr::left_join(ecoc_merged_all, flags, by = "hh_id") %>%
...
```{r bad_tests_1}
  #ecoc_merged$qc[ecoc_merged$hh_id == "IN10"] <- "bad"
```
...
```{r filter_test_times}
  ecoc_merged <- filter_times(times, ecoc_merged_all)
```
...
```{r get_pre_bkgd}
  ...
  pre_bkgd <- filter_times(times, ecoc_merged_all)
```
...
```{r get_post_bkgd}
  ...
  post_bkgd <- filter_times(times, ecoc_merged_all)
```
...
```{r merge_bkgd}
  ecoc_merged <- dplyr::left_join(ecoc_merged, bkgd, by = c("field_site", "hh_id", "date", "units", "qc"))
```
...
```{r plot_co2, fig.height=10, fig.width=10}
  field_timeseries_plot(ecoc_merged, "co2")
```

## boxplot co2

```{r boxplot_co2}
  field_boxplot(ecoc_merged, "co2")
```

## co

```{r plot_co, fig.height=10, fig.width=10}
  field_timeseries_plot(ecoc_merged, "co")
```

```{r boxplot_co}
  field_boxplot(ecoc_merged, "co")
```

## so2

```{r plot_so2, fig.height=10, fig.width=10}
  field_timeseries_plot(ecoc_merged, "so2")
```


```{r boxplot_so2}
  field_boxplot(ecoc_merged, "so2")
```

## vocs

```{r plot_vocs, fig.height=10, fig.width=10}
  field_timeseries_plot(ecoc_merged, "vocs")
```


```{r boxplot_vocs}
  field_boxplot(ecoc_merged, "vocs")
```

# Summary

Gas data was measured for `r length(unique(ecoc_merged$hh_id))` experiments between `r min(ecoc_merged$date, na.rm = TRUE)` and `r max(ecoc_merged$date, na.rm = TRUE)`. There is no gas data for tests: `r setdiff(as.character(samples$hh_id), as.character(ecoc_merged$hh_id))`.
...
```{r long_data_conversion}
  ecoc_merged <- dplyr::select(ecoc_merged, field_site, hh_id, date, time, datetime,
...
```{r save_data}
  saveRDS(ecoc_merged, file = "../r_files/field_temp_merged.rds")
```
