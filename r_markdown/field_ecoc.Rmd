...
## Correct for oc artifact

* select artifact filters

```{r}
  ecoc_artifact <- ecoc %>%
                   dplyr::filter(inst == "filter_2") %>%
                   dplyr::select(oc_ugsqcm, ec_ugsqcm, hh_id) %>%
                   dplyr::rename(oc_artifact = oc_ugsqcm, ec_artifact = ec_ugsqcm)
```

* merge artifact with sample data

```{r merge_calcs}
  ecoc <- ecoc %>%
          dplyr::left_join(ecoc_artifact, by = "hh_id")
```
...
```{r calc_delta_wgt}
  ecoc <- ecoc %>%
          dplyr::mutate(oc_corr = oc_ugsqcm - oc_artifact)
```
