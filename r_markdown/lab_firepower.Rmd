...
# Metrics by stove type

## Firepower

```{r, echo=FALSE, fig.height=4, fig.width=8}
  grav_merged %>%
    dplyr::filter(stove_cat == "traditional stoves",
                  fuel_type == "wood",
                  var == "fp") %>%
    dplyr::mutate(stove = gsub(" [:(:]", "\n (", stove)) %>%
    ggplot(aes_string(x = "stove", y = "val", color = "fuel")) +
      geom_boxplot(outlier.colour = NA) +
      scale_shape(solid = FALSE) +
      guides(fill = FALSE) +
      theme_bw() +
      theme(text = element_text(size = 16),
            axis.text.x = element_text(angle = 15, hjust = 1, vjust = 1, size = 8),
            legend.position = "top") +
      facet_wrap(fuel_type~stove_cat, ncol = 1, scales = "free") +
...
```{r, echo=FALSE, fig.height=4, fig.width=8}
  grav_merged %>%
    dplyr::filter(stove_cat == "traditional stoves",
                  ...
                  var == "fp") %>%
    dplyr::mutate(stove = gsub(" [:(:]", "\n (", stove)) %>%
    ggplot(aes_string(x = "stove", y = "val", color = "sample_id")) +
      geom_boxplot(outlier.colour = NA) +
      scale_shape(solid = FALSE) +
      guides(fill = FALSE) +
      theme_bw() +
  ...
  grav_merged %>%
    ...
    ggplot(aes_string(x = "stove", y = "val", color = "fuel")) +
      ...
      theme(text = element_text(size = 16),
            axis.text.x = element_text(angle = 15, hjust = 1, vjust = 1, size = 8),
            legend.position = "top") +
      facet_wrap(fuel_type~stove_cat, ncol = 2, scales = "free") +
      labs(color = "fuel type") +
      ylab("") +
      xlab("")
```
