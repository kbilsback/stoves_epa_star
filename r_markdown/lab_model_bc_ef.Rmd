...
The resulting linear model is: $$\text{BC rate} = `r round(basic_coef[2], digits = 3)` \times FP + `r round(basic_coef[1], digits = 3)`$$ with a *rmse* of `r round(basic_rmse, digits = 3)` g/kg.
...
The resulting linear model has a *rmse* of `r round(basic_rmse, digits = 3)` g/kg.
...
The resulting linear model has a *rmse* of `r basic_rmse_bc` in g/kg. 
...
The resulting mfp model has a *rmse* of `r basic_rmse` in g/kg. The *rmse* hasn't changed much but the variance explained improved greatly.
...
The resulting mfp model has a *rmse* of `r basic_rmse` in g/kg. 

The firepower variable is selected twice again the rmse amd variance explaiend don't change much.
...
There is little a meaningful change when mce and the mce and firepower interaction term are added.
...
When mce is the only predictor the resulting linear model has a *rmse* of `r round(basic_rmse, digits = 3)` g/kg. And the percent variance explained is very low. 
...
The resulting mfp model has a *rmse* of `r basic_rmse` in g/kg. Mce is selected as a significant variable. 
...
The resulting mfp model has a *rmse* of `r basic_rmse` in g/kg. Firepower is chosen in two forms and mce is not chosen as an important variable.
...
## Overall Conclusions:
Firepower seems to be a meaningful predictor for bc emissions factors.

Firepower is a better predictor than mce for bc emissions factors.
