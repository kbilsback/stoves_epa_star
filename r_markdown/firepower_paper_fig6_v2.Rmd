...
```{r}
  ...
  fp_scale <- 
    mod_data %>%
    dplyr::ungroup() %>%
    dplyr::summarise(mean = round(mean(fp), 2),
                     sd = round(sd(fp), 2))
  
  mce_scale <- 
      mod_data %>%
        dplyr::ungroup() %>%
        dplyr::summarise(mean = round(mean(mce), 2),
                         sd = round(sd(mce), 2))
  ...
  mod2 = lm(log(pm_ef) ~ mce , data = mod_data)
  mod3 = lm(log(pm_ef) ~ fp + mce , data = mod_data)
  mod4 = lm(log(pm_ef) ~ fp + stove, data = mod_data)
  mod5 = lm(log(pm_ef) ~ fp*stove , data = mod_data)
  mod6 = lm(log(pm_ef) ~ mce + stove , data = mod_data)
  mod7 = lm(log(pm_ef) ~ mce*stove , data = mod_data)
  mod8 = lm(log(pm_ef) ~ fp + stove + mce , data = mod_data)
  mod9 = lm(log(pm_ef) ~ fp*stove + mce, data = mod_data)
  mod10 = lm(log(pm_ef) ~ mce*stove + fp, data = mod_data)
...
```{r}
  saveRDS(mod5, "../r_files/wood_pm_mod.RDS")
  saveRDS(fp_scale, "../r_files/wood_scale.RDS")
```
...
```{r}
  saveRDS(mod4, "../r_files/char_pm_mod.RDS")
```
...
```{r}
  saveRDS(mod5, "../r_files/wood_bc_mod.RDS")
```
...
```{r}
  saveRDS(mod2, "../r_files/wood_co_mod.RDS")
```
...
```{r}
  saveRDS(mod3, "../r_files/char_co_mod.RDS")
```
