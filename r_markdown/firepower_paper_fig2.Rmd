...
```{r}
  p1 <-
    continuous %>%
      dplyr::filter(grepl("Three-stone|Rocket elbow|Ceramic plancha", stove)) %>%
      ggplot(aes(x = fp, y = mce)) +
        geom_point(aes(color = fuel), size = 2, alpha = 0.5) +
...
```{r figure_2, echo=FALSE, fig.width=12, fig.height=6, dpi=400, dev='png'}
