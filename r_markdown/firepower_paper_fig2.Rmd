...
```{r, fig.width=11, fig.height=4}
  continuous %>%
    dplyr::filter(grepl("three|rocket|ceramic", stove)) %>%
    ggplot(aes(x = fp, y = mce)) +
      geom_point(aes(color = fuel), shape = 1, size = 3, alpha = 0.75, stroke = 1) +
      #geom_text(data = eqn1 %>%
      #            dplyr::filter(grepl("three|rocket", stove)), aes(x = 10, y = 0.825, label = eq),
      #          color = "black",  parse = TRUE, size = 4) +
      #geom_text(data = eqn2 %>%
      #            dplyr::filter(grepl("three|rocket", stove)), aes(x = 9, y = 0.80, label = eq),
      #          color = "black",  parse = TRUE, size = 4) +
      geom_errorbar(aes(ymin = mce_min, ymax = mce_max),
                    width = 0, color = "black") +
      geom_errorbarh(aes(xmin = fp_min, xmax = fp_max),
...
```{r fig_2b, fig.width=11, fig.height=4}
  batch %>%
    ggplot(aes(x = fp, y = mce)) +
      geom_point(aes(shape = phase, color = fuel), size = 3, alpha = 0.75, stroke = 1) +
