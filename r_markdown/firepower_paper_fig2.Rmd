...
```{r fig_2a, fig.height=4, fig.width=11}
  ...
  grav %>%
    ...
    ggplot(aes(x = fp, y = mce)) +
      ...
      scale_y_continuous(name="combustion efficiency", limits=c(0.8, 1.0)) +
  ...
  grav %>%
    ...
    ggplot(aes(x = fp, y = mce)) +
      ...
      geom_point(data = wbt_batch, aes(x = firepower, y = mce,
                                         colour = fuel), shape = 8, size = 3,
                 stroke = 1.4) +
        scale_colour_manual(values = c( "darkorchid", "olivedrab1", "gray"),
                            guide = guide_legend(override.aes =
                                                   list(linetype = c(rep("blank", 5)),
                                                        shape = c(rep(15, 5))))) +
      theme_bw() + 
      theme(text = element_text(size = 16),
            plot.title = element_text(hjust = 0.5),
