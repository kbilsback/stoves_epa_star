...
```{r}
  ...
  mod1 = lm(log(pm_rate) ~ fp, data = mod_data)
  mod2 = lm(log(pm_rate) ~ fp + stove_fuel , data = mod_data)
  mod3 = lm(log(pm_rate) ~ fp*stove_fuel , data = mod_data)

  model.sel(mod1, mod2, mod3)
```

* Save top firepower model

```{r}
  saveRDS(mod3, "../r_files/wood_pm_rate_mod.RDS")
```

* R2 of top firepower model
...
```{r, fig.height=4, echo=FALSE}
  ggplot(data = mod_data, aes(y = log(pm_rate), x = fp)) +
...
```{r}
  saveRDS(mod3, "../r_files/charcoal_pm_rate_mod.RDS")
```
...
```{r}
  ...
  mod1 = lm(log(bc_rate) ~ fp , data = mod_data)
  mod2 = lm(log(bc_rate) ~ fp + stove_fuel , data = mod_data)
  mod3 = lm(log(bc_rate) ~ fp*stove_fuel , data = mod_data)
...
```{r}
  saveRDS(mod3, "../r_files/wood_bc_rate_mod.RDS")
```
