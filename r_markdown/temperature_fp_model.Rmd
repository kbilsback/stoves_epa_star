...
* Hasn't changed much between first- and second-order polynomial plots
...
* First- and second- order fixed-effect models perform roughly the same.
...
## Summarize field-derived temperature data
...
* Boxplot of temperature data by household
...
## Predict firepower from field-derived temperature data using lab-derived models
...
```{r}
  field_fp_pred <- predict(mod1, field_temp_m)
```
...
```{r}
  yhat <- matrix(field_fp_pred, ncol = 1)
```

Get covariance matrix for coefficients

```{r}
  varbetahat <- vcov(mod1)
...
```{r}
  Xmat <- model.matrix(~temp*stove, data = field_temp_m)
```
...
```{r}
  ...
  var_yhat <- round(Xmat %*% varbetahat %*% t(Xmat), 10)

  is.positive.definite(var_yhat) # this is ok
  is.symmetric.matrix(var_yhat) # good
```
...
```{r}
  ...
  field_fp_boot <- rmvnorm(B, yhat, var_yhat)
  dim(field_fp_boot)
```

Exponentiate bootstrapped estimates

```{r}
  field_fp_boot <- exp(field_fp_boot)
```

Reorganize data

```{r}
  field_fp_boot <- t(field_fp_boot)

  field_fp_boot <-  as.data.frame(field_fp_boot)

  field_fp_boot <-
    field_fp_boot %>%
    dplyr::bind_cols(dplyr::select(field_temp, hh_id, stove, datetime)) %>%
    tidyr::gather("rep", "val", 1:10000)
```
