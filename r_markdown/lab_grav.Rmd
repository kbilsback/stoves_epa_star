...
```{r plot_pm_wood, fig.height=30, fig.width=12, echo=FALSE}
  p_grav <- grav %>%
 ...
 ggplot(p_grav, aes_string(x = "sample_id", y = "pm_ef", fill = "fuel")) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 18), legend.position = "top")
```

```{r plot_pm_charcoal, fig.height=30, fig.width=12, echo=FALSE}
  p_grav <- grav %>%
...
```{r plot_pm_wood, fig.height=30, fig.width=12, echo=FALSE}
  p_grav <- grav %>%
            dplyr::filter(fuel_type == "gasifier") %>%
            dplyr::group_by(stove, fuel, sample_id) %>%
            dplyr::summarise(pm_ef = mean(pm_ef, na.rm = TRUE))
