...
field_sumsarized_events_all <-  dplyr::mutate(field_sumsarized_events_all,start_time = if_else(grepl("add 11.5",notes),
...
#If data has an end time, keep only the data between the start and end time, for the given household.
test <- field_sumsarized_events_all[((field_sumsarized_events_all$start_time<field_sumsarized_events_all$end_date & field_sumsarized_events_all$start_time>field_sumsarized_events_all$start_date) | is.na(field_sumsarized_events_all$end_date)),]
...
field_sumsarized_timeseries_all <-filter_sum_data(field_sumsarized_timeseries,field_temp_meta,bad_files)
field_sumsarized_timeseries_all <- dplyr::mutate(field_sumsarized_timeseries_all,datetime = if_else(grepl("add 11.5",notes),
                              datetime+hours(11) + minutes(30), datetime))  #If datapoint has a comment with the text 11.5, add that to the timestamp.
                              dplyr::group_by(event_num) %>% #There are some duplicate cooking event numbers because during the join between the meta data and the data set, it was not clear which sum was used to generate the cooking event...but it does not matter, because the stove and hhid were the same.  This was the case for Uganda data that had the hhid in the filename and was used for matching rather than the sum id.
                              dplyr::filter(row_number() == 1) %>%
                              dplyr::ungroup()
...
# #Initialize the cooking_events frame.
 ...
 for (i in 1:max(field_sumsarized_timeseries_all$file_indices)) {
