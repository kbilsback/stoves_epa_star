...
```{r define_variables}
  stove_codes <- data.frame(stove = as.factor(c("Metal jiko",
            ...
            stove_descriptions = as.factor(c("Metal jiko", "Ceramic coalpot", "Ceramic jiko",
                  "Ceramic jiko","Ceramic jiko","Kerosene", "Tire rim","Traditional chimney",
...
field_sumsarized_events_all <-dplyr::left_join(field_sumsarized_events_all,
                                    dplyr::select(field_sumsarized_csvdurations, filename,
...
# #Initialize the cooking_events frame.
 cooking_events_timeseries <- data.frame(start_time=as.POSIXct(character()),
                    ...
                    file_indices = as.numeric(),events_per_file =
                    as.numeric(),stove_use_category=factor(),use_flag = as.logical())
 ...
 for (i in unique(field_sumsarized_timeseries_all$file_indices)) {
   ...
   if (dim(temp)[1]>1) {
      ...
      breakend <- c(which((difftime>cooking_group) == TRUE),
                    ...
                    #Tail part is in case the time series ends while still cooking...need to account for that.
      ...
      cooking_events_timeseries <- rbind(cooking_events_timeseries,
         ...
         #If not cooking events are found, still create an entry, though with the use flag as FALSE, so 
...
```{r create_flags}
  ...
  ok_cooking_events <- dplyr::filter(all_cooking_events,grepl("ok",qc,ignore.case=TRUE)) 
...
```{r fix_timezones}
ok_cooking_events <- dplyr::mutate(ok_cooking_events,
                                        datetime = lubridate::force_tz(start_time,
                                                                       tzone = "Asia/Calcutta"))
```
...
gg <- ggplot(events_summary, aes(x=stove_descriptions, y = events_per_day)) + 
  stat_summary(fun.data = f, geom="boxplot") +  geom_point() +
...
plotDensFunc <- function(x,y, na.rm = TRUE, ...) {
    plots <- ggplot(x, aes(start_time, fill = stove_descriptions, 
...
```{r plot_all_temp_data, fig.width=12, fig.height=20}
  field_timeseries_plot(ok_cooking_events, "duration_minutes")
```
...
```{r long_data_conversion}
  field_cooking_events_all <- dplyr::select(all_cooking_events, 
                          field_site, hh_id, start_time, duration_minutes,stove_descriptions,stove_use_category,units, qc) %>%
...
```{r save_data}
  saveRDS(field_cooking_events_all, file = "../r_files/field_cooking_events_all")
```
