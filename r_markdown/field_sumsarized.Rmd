...
datalist = list()
retrospection <- 48#number of hours to look back and forwards.

temp2 <- dplyr::group_by(ok_cooking_events,stove_use_category,hh_id)
...
events_honduras_behavior <- dplyr::filter(ok_cooking_events,grepl("honduras",field_site,ignore.case=TRUE)) %>%
 ...
 for (i in 1:dim(honduras_behavior)[1]) {
  temp <- dplyr::filter(ok_cooking_events,grepl(honduras_behavior$house_id[i],hh_id)) %>%
          dplyr::filter(start_time<honduras_behavior$date[i]+3600*retrospection & start_time>honduras_behavior$date[i]-3600*retrospection) %>% #POSIXct deals in seconds, thus the 3600 converting from hours to seconds.
...
gg4 <- ggplot(events_honduras_behavior, aes(x=usage_events, y = survey_events)) + 
  ...
  xlim(-0, 13) +  ylim(-0, 13) 
gg4
ggsave(filename="figures/Honduras_survey_hours_b48.pdf", plot=gg4)
