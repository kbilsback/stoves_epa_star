...
gg3 <- ggplot(honduras_behavior_compare, aes(x=usage_hours, y = survey_hours)) + 
  ...
  theme_bw()+
  xlim(-0, 13) +  ylim(-0, 13) + 
  geom_text(aes(label=ifelse(survey_hours>7.5 | survey_hours<1.5,as.character(stove_descriptions),'')),
            angle = 60,hjust=0,vjust=0,size=3)
gg3
ggsave(filename="figures/Honduras_survey_hours_c.pdf", plot=gg3)
...
gg4 <- ggplot(honduras_behavior_compare, aes(x=usage_events, y = survey_events)) + 
  ...
  xlim(-0, 13) +  ylim(-0, 13) +   
  theme_bw()+
  geom_text(aes(label=ifelse(survey_events>6 | survey_hours<1,as.character(stove_descriptions),'')),
            angle = 60,hjust=0,vjust=0,size=3)
gg4

ggsave(filename="figures/Honduras_survey_hours_d.pdf", plot=gg4)
```
...
s  saveRDS(field_sumsarized_timeseries_all, file = "../r_files/field_sumsarized_timeseries_all.RDS")
