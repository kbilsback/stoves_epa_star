...
```{r load_data}
  field_sumsarized_events <- readRDS("../r_files/field_sumsarized_events.RDS")
  field_sumsarized_csvdurations <- readRDS("../r_files/field_sumsarized_durations.RDS")
```
...
#For uganda data, find which files are from Uganda, and find the hh_id in the file name.  Then join with the meta data to get the logger id, field_site, and stove_type
  field_sumsarized_events_uganda <-  dplyr::filter(field_sumsarized_events,grepl("Uganda",filename, fixed=TRUE)) %>%
...
#Need to deal with duplicate loggers on a stove
  field_sumsarized_events_honduras <-  dplyr::filter(field_sumsarized_events,grepl("honduras",filename, fixed=TRUE)) %>%
...
#Need to deal with duplicate loggers on a stove.                       
    field_sumsarized_events_china <-  dplyr::filter(field_sumsarized_events,!grepl("Uganda|India|honduras",filename)) %>%
                             dplyr::mutate(hh_id = substring(filename, 
                                    sapply(filename, function(x) unlist(gregexpr('_',x,perl=TRUE))[3])+1, 
                                    sapply(filename, function(x) unlist(gregexpr('_',x,perl=TRUE))[4])-1)) %>%  
                             dplyr::mutate(stove = substring(filename, 
    ...
    #Need to deal with duplicate loggers on a stove.  
      field_sumsarized_events_india <-  dplyr::filter(field_sumsarized_events,grepl("India",filename, fixed=TRUE)) %>%
                            dplyr::mutate(logger_id = substring(filename, 
...
# for (i in 1:max(field_sumsarized_all$file_indices)) {
...
field_sumsarized_events_all <- 
        dplyr::left_join(field_sumsarized_events_all,
...
events_summary <- dplyr::filter(field_sumsarized_events_all,!duplicated(filename)) %>%
...
```{r fix_timezones}
  field_sumsarized_events_all <- dplyr::mutate(field_sumsarized_events_all,
...
plotDensFunc(dplyr::filter(field_sumsarized_events_all,!grepl("Uganda|India|honduras",filename)),"China")
plotDensFunc(dplyr::filter(field_sumsarized_events_all,grepl("India",filename)),"India")
plotDensFunc(dplyr::filter(field_sumsarized_events_all,grepl("Uganda",filename)),"Uganda")
plotDensFunc(dplyr::filter(field_sumsarized_events_all,grepl("honduras",filename)),"Honduras")
...
```{r merge_flags}
  # fix warning
  field_sumsarized_events_all <- dplyr::left_join(field_sumsarized_events_all, flags, by = "hh_id") %>%
...
```{r bad_tests_2}
  field_sumsarized_events_all$qc[field_sumsarized_events_all$hh_id == "IN4"] <- "maybe"
```
