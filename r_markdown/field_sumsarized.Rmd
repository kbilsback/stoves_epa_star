...
matched <- intersect(field_sumsarized_durations_csvs$filename, field_sumsarized_events_all_temp$filename)
all <-  union(field_sumsarized_durations_csvs$filename, field_sumsarized_events_all_temp$filename)
...
honduras_behavior_comparison_time <- dplyr::filter(ok_cooking_events_honduras,grepl("honduras",field_site,ignore.case=TRUE)) %>%
          ...
          dplyr::mutate(hhid_category = if_else(grepl("primary",stove_use_category,ignore.case=TRUE),paste0(hh_id,"primary"),
              if_else(grepl("secondary",stove_use_category,ignore.case=TRUE),paste0(hh_id,"secondary"),
          ...
          dplyr::rename('sums cooking time' = sums_cooking_time,'survey time'  = survey_hours) %>%
          dplyr::distinct(hhid_category, category, .keep_all = TRUE) %>%
          tidyr::gather(key = category,value =hours, 'sums cooking time' ,'survey time')
...
primary_hours <-dplyr::filter(honduras_behavior_comparison_time,stove_use_category=="Primary") %>%
  dplyr::filter(!is.na(hours)) %>% dplyr::filter(!is.na(stove_use_category)) 
kruskal.test(as.vector(primary_hours$hours),as.factor(primary_hours$category))
t.test(hours~category,data=primary_hours)

secondary_hours <-dplyr::filter(honduras_behavior_comparison_time,stove_use_category=="Secondary") %>%
  dplyr::filter(!is.na(hours))
kruskal.test(as.vector(secondary_hours$hours),as.factor(secondary_hours$category))
t.test(hours~category,data=secondary_hours)

ggsave(filename="figures/Honduras_survey_hours_a48.pdf", plot=gg3,width = 10, height = 5)
