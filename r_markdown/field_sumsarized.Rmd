...
```{r define_variables}
  stove_codes <- data.frame(stove = as.factor(c("Metal jiko",
            ...
            stove_descriptions = as.factor(c("Metal jiko", "Ceramic coalpot", "Ceramic coalpot",
                  "Ceramic coalpot","Ceramic coalpot","Kerosene", "Tire rim","Traditional chimney",
                  ...
                  "Rocket","Chimney","Three stone fire","Chimney Secondary","Three stone fire","Electric",
                  "LPG","Traditional")))
...
```{r create_flags}
  ok_cooking_events <-  dplyr::mutate(all_cooking_events,
                          ...
                          dplyr::mutate(month_year = format(start_time,"%m-%y")) %>%
                          ...
                          dplyr::distinct(start_time,duration_minutes,hh_id,stove, .keep_all = TRUE)  #There are some cases that have duplicated events due to overlapping files.  (ie. file started on1/1 and was downloaded on 1/5, but then redeployed without deleting, and redownloaed on 1/9.  So duplicated events from 1/1 to 1/5.  )
...
ok_cooking_events <- 
        dplyr::group_by(ok_cooking_events,hh_id,stove) %>%   #group_by is handy.
...
#Summary of all cooking events.
 grouped_summary_a <- ok_cooking_events %>% dplyr::group_by(field_site,stove_descriptions) %>%
      dplyr::summarize(mean=mean(duration_minutes, na.rm = TRUE),
                       median=median(duration_minutes, na.rm = TRUE),
                       sd=sd(duration_minutes, na.rm = TRUE)) 
 
grouped_summary_b <- ok_cooking_events %>% dplyr::group_by(field_site,stove_descriptions) %>%
      dplyr::filter(!duplicated(filename)) %>% #Remove dup filenames to get these stats below without duplication.
      dplyr::summarize(days_logged=sum(logging_duration_days, na.rm = TRUE),
                       households=n())

#Summarize use by household/deployment/stove for plotting average use.
events_summary <- dplyr::filter(ok_cooking_events,!duplicated(filename)) %>%
        ...
        dplyr::filter(!duplicated(hh_id) & !duplicated(stove))

#Get average of average daily uses by hh and stove
grouped_summary_c <- events_summary %>% dplyr::group_by(field_site,stove_descriptions) %>%
...
kable(grouped_summary_c, digits=2)
...
#Get nhouses_logging the old fashioned way of using a loop. 
        dplyr::mutate(percent_stove_days = stove_days/nhouses_logging*100) %>%
...
gg <- ggplot(events_summary, aes(x=stove_descriptions, y = events_per_day)) + 
  stat_summary(fun.data = f, geom="boxplot") +  geom_point(position="jitter",alpha = 0.25) +
...
gg2 <- ggplot(events_summary, aes(x=stove_descriptions, y = logging_duration_days)) + 
  stat_summary(fun.data = f, geom="boxplot") + geom_point(position="jitter") +
  facet_grid(~field_site,scales = "free", space = "free") + 
...
monthlyboxplots <- ggplot(events_summary_monthly, aes(x=month_year, y = events_per_day_month,color = stove_descriptions)) + 
  stat_summary(fun.data = f, geom="boxplot",position = position_dodge(width=0.9)) +
  geom_point(position=position_jitterdodge(dodge.width=0.9),alpha = 0.3) +
  facet_grid(~field_site,scales = "free", space = "free") + 
  labs(y="Mean events/day",x="") + 
  theme(axis.text.x = element_text(angle = 60, hjust = 1))
monthlyboxplots
ggsave(filename="figures/monthlyboxplots.pdf", plot=monthlyboxplots)
...
```{r honduras_behavior_comparison}
  ...
  #honduras_behavior data has the date set at midnight UTC, so it looks like the survey date is coarse, so I will look back 24 hours initially to ensure that I get the previous 24h of usage, and forward 36h for the same reason.  There is no risk here as the data files were already well organized and would not have long durations with a mislabeled file name.
datalist = list()

 for (i in 1:dim(honduras_behavior)[1]) {
  temp <- dplyr::filter(ok_cooking_events,grepl(honduras_behavior$house_id[i],hh_id)) %>%
          dplyr::filter(start_time<honduras_behavior$date[i]+3600*24 & start_time>honduras_behavior$date[i]-3600*24) %>%
...
honduras_behavior_compare$survey_hours[is.na(honduras_behavior_compare$survey_hours)] <- 0
honduras_behavior_compare <- honduras_behavior_compare[!honduras_behavior_compare$survey_events==999,] 

```

```{r plot_behavior_comparison}
gg3 <- ggplot(honduras_behavior_compare, aes(x=usage_hours, y = survey_hours)) + 
  geom_point(alpha = 0.2) + #stat_binhex()
  labs(y="24h recall reported cooking time (hours)",x="24h SUM cooking time (hours)") +
  theme(axis.text.x = element_text(angle = 60, hjust = 1)) + 
  ...
  theme(plot.title = element_text(hjust = 0.5)) + 
  xlim(-0, 13) +  ylim(-0, 13) + 
  geom_text(aes(label=ifelse(survey_hours>7.5 | survey_hours<1.5,as.character(stove_use_category),'')),
            angle = 60,hjust=0,vjust=0,size=3)
gg3
ggsave(filename="figures/Honduras_survey_hours_a.pdf", plot=gg3)

gg4 <- ggplot(honduras_behavior_compare, aes(x=usage_events, y = survey_events)) + 
  geom_point(alpha = 0.1) + #stat_binhex()
  labs(y="24h recall reported cooking events",x="24h SUM events") + 
  theme(axis.text.x = element_text(angle = 60, hjust = 1)) +
  theme(plot.title = element_text(hjust = 0.5)) + 
  ...
  ggtitle("Honduras reported vs. measured stove use") +
  xlim(-0, 13) +  ylim(-0, 13) + 
  geom_text(aes(label=ifelse(survey_events>6 | survey_hours<1,as.character(stove_use_category),'')),
            angle = 60,hjust=0,vjust=0,size=3)
gg4

ggsave(filename="figures/Honduras_survey_hours_b.pdf", plot=gg4)
```
...
```{r save_data}
  saveRDS(ok_cooking_events, file = "../r_files/field_ok_cooking_events")
```
