...
```{r}
  field_plot_1 <-
    ...
    dplyr::filter(grepl("co_ef|pm_ef|bc_ef", var),
                  grepl("chimney|three stone fire|ceramic jiko|forced|rocket elbow", stove)) %>%
...
```{r}
  field_plot_2 <-
    ...
    dplyr::filter(grepl("co|pm|ec", pol),
                  grepl("mean", var),
                  grepl("chimney|rocket elbow|three stone fire|draft", stove)) %>%
...
```{r}
  ...
  field_plot_4 <-
    ...
    dplyr::filter(grepl("three|metal jiko|rocket|forced|chimney|ceramic jiko", stove)) %>%
