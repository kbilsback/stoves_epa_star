...
```{r, fig.width=10, fig.height=4}
  continuous %>%
  dplyr::filter(grepl("three|rocket|mud", stove)) %>%
    ggplot(aes(x = fp, y = (pm_ef), color = fuel)) +
      geom_point(shape = 1, size = 3, alpha = 0.75, stroke = 1) +
      #geom_smooth(method = "lm", se = FALSE) +
      geom_text(data = fir_r2 %>%
                dplyr::filter(grepl("three|rocket|mud", stove)),
                              aes(x = 7.5, y = 70, label = eq),
                color = "#999999",  parse = TRUE, size = 5.5) +
      geom_text(data = euc_r2 %>%
                dplyr::filter(grepl("three|rocket|mud", stove)),
                              aes(x = 7.5, y = 25, label = eq),
                color = "#E69F00",  parse = TRUE, size = 5.5) +
      geom_text(data = cont_r2 %>%
                dplyr::filter(grepl("three|rocket|mud", stove)),
                              aes(x = 4.0, y = 70, label = eq),
                color = "black",  parse = TRUE, size = 5.5) +
      scale_color_manual(values = c("#999999", "#E69F00")) +
      scale_y_continuous(breaks = c(0.1, 1, 10, 100),
                         limits = c(0.05, 100), trans = "log10") +
      scale_x_continuous(breaks = c(0, 2, 4, 6, 8, 10, 12, 14), limits = c(0, 10)) +
      guides(color = guide_legend(override.aes = list(size = 3, alpha = 0.75,
                                                      shape = 1, linetype = 0))) +
      theme_bw() +
      theme(text = element_text(size = 16),
