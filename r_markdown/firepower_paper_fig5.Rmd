...
```{r}
  field_plot_1 <-
    ...
    dplyr::mutate(fuel = gsub(".*doug.*", "douglas fir (milled)", fuel),
                  ...
                  stove = gsub("forced draft gasifier", "forced draft\ngasifier", stove)) %>%
...
```{r}
  field_plot_2 <-
    ...
    dplyr::select(stove, fuel, co_ef, pm_ef, bc_ef) %>%
    dplyr::mutate(source = ifelse(grepl("gasifier|three", stove),
                                  "Wathore et al., 2017",
                                  "Roden et al., 2006 & 2008"))
```
...
```{r}
  field_plot_3 <-
    ...
    dplyr::mutate(stove = ifelse(stove != "justa", "ceramic jiko", stove),
                  stove = gsub(".*justa.*", "traditional plancha\n(chimney)", stove),
                  fuel = "wood (collected)") %>%
    dplyr::mutate(source = ifelse(grepl("honduras", field_site),
                                  "Honduras (2016)",
                                  "Uganda (2016)")) %>%
