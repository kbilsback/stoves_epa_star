...
```{r}
  field_plot_1 <-
    ...
    dplyr::filter(grepl("co_ef|pm_ef|bc_ef", var),
                  grepl("plancha*|three stone fire|ceramic jiko|forced|rocket elbow", stove)) %>%
...
```{r}
  field_plot_2 <-
    ...
    dplyr::filter(grepl("co|pm|ec", pol),
                  grepl("mean", var),
                  grepl("plancha*|rocket elbow|three stone fire|draft", stove)) %>%
...
```{r,fig.width=8.5, fig.height=4}
  field_plot %>%
  dplyr::filter(!grepl("metal", stove)) %>%
  ggplot(aes(y = pm_ef, x = test_type)) +
    geom_jitter(aes(color = fuel), width = 0.15, alpha = 0.75, size = 1) +
...
```{r,fig.width=8.5, fig.height=4}
  ...
  ggplot(aes(y = pm_ef, x = stove)) +
    ...
    theme(text = element_text(size = 14),
          ...
          strip.text.x = element_text(size = 9),
          legend.position = "top") +
    facet_grid(~test_type, space = "free", scale = "free") +
    scale_colour_manual(values = c("#F0E442", "#999999", "#CC79A7", "#E69F00",
                                   "#009E73", "#56B4E9", "#0072B2")) +
        guides(color = guide_legend(override.aes = list(linetype = 0, size = 3))) +
    xlab("") +
...
```{r, fig.width=8, fig.height=4}
  field_plot %>%
  dplyr::filter(!grepl("jiko", stove), test_type != "WBT") %>%
