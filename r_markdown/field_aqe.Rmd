...
```{r merge_bkgd}
  aqe_merged <- dplyr::left_join(aqe_merged,
                                 dplyr::select(bkgd, -field_site, -hh_id, -qc, -units), by = c("date")) %>%
                dplyr::distinct()  # need to figure out why there are duplicates
```

* update qc flags

```{r update_qc_flags}
  flags <- dplyr::group_by(aqe_merged, hh_id) %>%
           dplyr::arrange(qc) %>%
           dplyr::summarise(qc = first(qc)) %>%
           dplyr::union(dplyr::select(bkgd, hh_id, qc), by = "hh_id") %>%
           dplyr::arrange(qc) %>%
           dplyr::group_by(hh_id) %>%
           dplyr::summarise(qc = first(qc))

  aqe_merged <- dplyr::select(aqe_merged, -qc) %>%
