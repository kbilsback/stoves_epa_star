...
```{r sample_merge}
  aqe_merged <- dplyr::mutate(aqe, date = replace(date, date == "2016-04-25", "2016-04-26")) %>%
                dplyr::mutate(aqe, date = replace(date, date == "2016-04-24", "2016-04-25")) %>%
                dplyr::mutate(aqe, date = replace(date, date == "2016-04-22", "2016-04-23")) %>%
                dplyr::mutate(aqe, date = replace(date, date == "2016-04-21", "2016-04-22")) %>%
                dplyr::mutate(aqe, date = replace(date, date == "2016-04-20", "2016-04-21")) %>%
...
```{r fix_datetime}
  aqe_merged <- dplyr::arrange(aqe_merged, datetime) %>%
                dplyr::distinct() %>%
                #dplyr::mutate(datetime = ifelse(field_site == "india",
                                                recast_tz(datetime, tz = "Asia/Calcutta"),
                                                datetime)) %>%
...
```{r tidy_aqe}
  aqe_merged <- dplyr::left_join(aqe_merged, 
                                 dplyr::select(samples, hh_id, field_site, date),
                                 by = "date") %>%
                dplyr::select(field_site, hh_id, date, time, datetime, co2, co, so2, vocs) %>%
...
* fix India dates and timezone

```{r fix_times}
  aqe_merged <- dplyr::mutate(aqe_merged, datetime = ifelse(field_site == "india",
                                              recast_tz(datetime, tz = "Asia/Calcutta"),
                                              datetime))%>%
...
Gas data is expected to be missing for IN12.
...
```{r save_data}
  saveRDS(aqe_merged, file = "../r_files/field_temp_merged.rds")
```
