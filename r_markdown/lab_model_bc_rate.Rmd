...
r2 <- lab_data %>%
      ...
      dplyr::do(eqn = unclass(lm_fp(eqn, .))) %>% 
      dplyr::mutate(eqn = unclass(eqn))
```

## Plot model

```{r fp_bc_rate, echo=FALSE}
  ggplot(lab_data, aes(x = fp, y = bc_rate)) +
    geom_point(size = 4) +
          theme_bw() + 
          facet_wrap(~stove, scales = "free", nrow = 2) +
          geom_smooth(method = "lm", formula = 'y ~ x',
                      color = 'black', se = FALSE) + 
    geom_text(data = r2, aes(x = -Inf, y = Inf, label = eqn), 
              color = 'black',  parse = TRUE,
              vjust = 1.5, hjust = "inward",
              size = 5) +
    theme(text = element_text(size = 20)) +
    ylab("Black Carbon Emissions Rate [mg/min]") + 
    xlab("Firepower [kW]")
```

## Model setup (modified combustion efficiency)
...
r2 <- lab_data %>%
      ...
      dplyr::do(eqn = lm_fp(eqn, .))
```

## Plot model

```{r mce_bc_rate, echo=FALSE}
  ggplot(lab_data, aes(x = mce, y = bc_rate)) +
    geom_point(size = 2) +
          theme_bw() + 
