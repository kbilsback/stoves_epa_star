...
```{r}
  ams <- readRDS("../../../r_files/lab_ams.RDS")
  ...
  bg %>%
    ...
    ggplot(aes_string(x = "date", y = "val")) +
      ...
      theme(text = element_text(size = 12),
            legend.position = "top") +
      ylab("OA (microgram per cubic meter)") +
      xlab("")
```
...
```{r}
  saveRDS(bg, file = "../../../r_files/ams_bg.RDS")
```

## plot: background and burn concentrations

```{r ams_burn_bg_conc, echo=FALSE}
  ams_merged %>%
...
```{r ams_trace_co2, echo=FALSE, fig.height=40, fig.width=12}
  ams_merged_p <-
    ams_merged %>%
...
```{r}
  ...
  saveRDS(ams_merged, file = "../../../r_files/ams_merged.RDS")
```

## plot: concentrations by stove type

```{r ams_stovetype_avg, echo=FALSE}
  ams_merged_p <-
    ams_merged %>%
    dplyr::left_join(dplyr::select(samples, id, stove, fuel), by = "id")
...
```{r ams_merged_fueltype, echo=FALSE}
  ams_merged_p %>%
