...
```{r load_libraries, echo=FALSE, message=FALSE}
  ...
  pacman::p_load(tidyverse, patchwork, weathermetrics, lme4, MuMIn, hms, xts)
...
```{r, echo=FALSE}
  ...
  firepower <- readRDS("../../r_files/lab_firepower_mce.RDS")
...
```{r}
  firepower <- 
    firepower  %>%
    dplyr::mutate(datetime = as.POSIXct(paste0(date, time))) %>%
    dplyr::select(id, stove, fuel, datetime, firepower, mce, fst_type) %>%
    dplyr::group_by(id, stove, fuel, fst_type, datetime = cut(datetime, breaks = "1 min")) %>%
...
```{r}
  saveRDS(temp_fp, "../../r_files/lab_temp_fp_mce_merged.RDS")
```