...
```{r load_data}
  temp <- readRDS("../../r_files/field_temp_uganda.RDS")
```
...
```{r}
  temp_merged_all <-
    ...
    dplyr::filter(!(hh_id == "UGA1" & date == "2015-06-29" & time < 54000),
                  !(hh_id == "UGA2" & date == "2015-07-02" & time < 36000),
                  !(hh_id == "UGA3" & date == "2015-07-03"),
                  !(hh_id == "UGA2" & date == "2015-07-04"),
                  temp > 0) #%>%
    #dplyr::select(-date)
```
...
```{r}
  temp_merged_all <-
    ...
    dplyr::mutate(datetime = as.POSIXct(datetime),
                  datetime = with_tz(datetime, tz = "Africa/Kampala"),
                  date = as.Date(datetime, tz = "Africa/Kampala"),
                  time = as.numeric(lubridate::hms(format(datetime, "%H:%M:%S"))))
```

```{r, fig.height=2, fig.width=12}
  temp_merged_all %>%
    dplyr::filter(hh_id == "UGA2") %>%
...
```{r, eval=FALSE, fig.height=10}
  temp_merged_all %>%
    ggplot(aes(x = datetime, y = temp)) +
      geom_line() +
      facet_wrap(~hh_id, scales = "free", nrow = 10)
```
...
```{r, fig.height=10}
  temp_merged_all %>%
    ggplot(aes(x = datetime, y = temp)) +
      geom_line() +
      facet_wrap(~hh_id, ncol = 1, scales = "free_y")
```
...
```{r save_data}
  saveRDS(temp_merged_all, file = "../r_files/uganda_temp_merged_all.RDS")
  saveRDS(temp_merged, file = "../r_files/uganda_temp_merged.RDS")
```
