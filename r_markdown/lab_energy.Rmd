...
```{r}
  energy <- 
    temp %>%
    dplyr::filter(!grepl("^6[A-Z]|^2[A-Z]", test_id)) %>%
    dplyr::left_join(water_mass, by = c("test_id", "sample_id")) %>%
    dplyr::mutate(energy_heat = total * c_water * delta_t,
                  energy_vap = loss * hfg_water,
                  energy_delivered = energy_heat + energy_vap) %>%
    dplyr::select(test_id, sample_id, energy_delivered) %>%
    dplyr::filter(!is.na(energy_delivered)) %>%
    dplyr::mutate(energy_delivered = energy_delivered,
                  units = "joules delivered")
```

```{r}
  ggplot(energy, aes(x = sample_id, y = energy_delivered, color = sample_id)) +
    geom_point(na.rm = TRUE) +
    theme_bw() +
    facet_wrap(~test_id, scales = "free") +
    theme(legend.position = "top") +
    ylab("joules delivered")
```

```{r, fig.height=20}
  saveRDS(energy, "../r_files/energy_delivered.RDS")
```
