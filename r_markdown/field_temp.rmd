...
```{r filter_tests}
  field_temp_merged <- filter_times(field_times, field_temp_merged_all)
```
...
```{r long_data_conversion}
  field_temp_merged_all <- dplyr::select(field_temp_merged_all, field_site, hh_id, date, time, datetime, temp, units, qc) %>%
                           tidyr::gather("var", "val", 6)

  field_temp_merged <- dplyr::select(field_temp_merged, field_site, hh_id, date, time, datetime, temp, units, qc) %>%
                       tidyr::gather("var", "val", 6)
```
