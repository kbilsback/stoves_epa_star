...
Temperature was measured for `r length(unique(sums_all$hh_id))` experiments between `r min(sums_all$date, na.rm = TRUE)` and `r max(sums_all$date, na.rm = TRUE)`. There is no temperature data for tests: `r setdiff(as.character(temp_meta$hh_id), as.character(temp_meta$hh_id))`.
