...
```{r combine_save}
  emissions <- dplyr::bind_rows(aqe, grav, temp)
```
...
```{r emissions_qc}
  emissions$qc[emissions$hh_id == "IN19" &  emissions$var == "avg"] <- "bad"
...
```{r bind_efs}
  emissions <- dplyr::bind_rows(emissions, pm25_ef)
```
...
```{r, echo=FALSE}
  p_emissions <- emissions %>%
                 dplyr::filter(var == "pm25_ef" | var == "avg_temp") %>%
                 dplyr::select(-pol, -units, -qc) %>%
                 tidyr::spread(var, val) %>%
                 dplyr::left_join(dplyr::select(samples, hh_id, stove_type))

    ggplot(p_emissions, aes(x = avg_temp, y = pm25_ef, colour = stove_type)) + 
    geom_point() + 
    theme_bw() +
    theme(legend.position = "top",
          text = element_text(size = 20),
...
```{r, echo=FALSE}
  p_emissions <- emissions %>%
                 dplyr::filter(var == "pm25_ef" | var == "prct95") %>%
                 dplyr::select(-pol, -units, -qc) %>%
                 tidyr::spread(var, val) %>%
                 dplyr::left_join(dplyr::select(samples, hh_id, stove_type))

    ggplot(p_emissions, aes(x = prct95, y = pm25_ef, colour = stove_type)) + 
    geom_point() + 
    theme_bw() +
    xlab("average/95th percentile") +
    ylab("pm25_ef")
    theme(legend.position = "top",
          text = element_text(size = 20),
          axis.text.x = element_text(angle = 45, hjust = 1, size = 12))
