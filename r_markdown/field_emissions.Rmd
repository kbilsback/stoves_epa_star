...
```{r combine_save}
  emissions <- dplyr::bind_rows(aqe, grav, temp, bc)
```
...
```{r calc_pm}
  pm25_ef <- emissions %>%
             ...
             dplyr::group_by(hh_id) %>%
             dplyr::mutate(pm_ef_fuel = (mass_conc * carbon) / total_carbon_conc) %>%
             dplyr::mutate(pm_ef_rate = pm_ef_fuel * (fuel_wgt / dur) * 1000) %>%
             dplyr::mutate(pm_ef_energy = (pm_ef_fuel * 1000) / (lhv / 1000)) %>%
             dplyr::mutate(firepower = (lhv * fuel_wgt) / (dur * 60)) %>%
             dplyr::select(hh_id, pol, qc, pm_ef_fuel, pm_ef_energy, pm_ef_rate, firepower) %>%
...
```{r calc_bc}
    bc_ef <- emissions %>%
             ...
             dplyr::left_join(meta, by = "hh_id") %>%
             dplyr::group_by(hh_id) %>%
             dplyr::mutate(bc_ef_fuel = (mass_conc * carbon) / total_carbon_conc) %>%
             dplyr::mutate(bc_ef_rate = bc_ef_fuel * (fuel_wgt / dur) * 1000) %>%
             dplyr::mutate(bc_ef_energy = (bc_ef_fuel * 1000) / (lhv / 1000)) %>%
             dplyr::select(hh_id, pol, qc, bc_ef_fuel, bc_ef_energy, bc_ef_rate) %>%
...
## Firepower vs. Avg flue temp normalized by 95th percentile
