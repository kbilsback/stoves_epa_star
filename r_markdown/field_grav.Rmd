---
title: "gravimetric"
output:
  html_document:
    toc: true
    toc_float: true
    theme: united
---

```{r global_options, include=FALSE}
  knitr::opts_chunk$set(fig.path='figures/', warning=FALSE, message=FALSE, cache=TRUE)
```

```{r load_libraries}
  library(tidyverse)
  library(forcats)
```

```{r load_functions}
  #source("../r_scripts/R_tidy.r")
```

# Load data

* gravimetric

```{r load_data}
  grav <- readRDS("../r_files/field_grav.rds")
```

* metadata

```{r load_meta}
  test_times <- readRDS("../r_files/field_test_times.rds")
  samples <- readRDS("../r_files/field_samples.rds")
  notes <- readRDS("../r_files/field_compiled_notes.rds")
  flows <- readRDS("../r_files/field_inst_flows.rds")
  filter_meta <- readRDS("../r_files/field_filter_meta.rds")
```

# Organize

## Filter grav data

* select only filters from emission tests that were used

```{r filter_grav}
  grav_merged <- dplyr::filter(grav, grepl("^IN[0-9]", id))
```

## Filter meta data

* select only data relevant for grav

```{r filter_meta}
  grav_meta <- dplyr::filter(filter_meta, !is.na(teflon_id)) %>%
               dplyr::filter(!grepl("Dummy|Jar Blank|Background", filter_type)) %>%
               dplyr::select(-quartz_id, -cart_type, -hh_id, -notes) %>%
               dplyr::left_join(dplyr::select(test_times, hh_id, date), by = "date") %>%
               dplyr::mutate(filter_type = fct_drop(filter_type))  # drop unused factor lvls
```

## Combine meta data

* calculate average flow rates and combine with test times

```{r meta}
 meta <- dplyr::filter(flows, inst == "filter_2") %>%
         dplyr::select(-notes) %>%  # go off bubble meter measures only
         dplyr::group_by(hh_id) %>%
         dplyr::summarise(avg_flow = mean(val, na.rm = TRUE)) %>%
         dplyr::mutate(filter_type = "Uncontrolled Test") %>%
         dplyr::left_join(dplyr::select(test_times, -date, -field_site), by = "hh_id")
```

## Merge data with meta

```{r meta_merge}
  grav_merged <- dplyr::left_join(grav_merged, grav_meta, by = c("id" = "teflon_id")) %>%
                 dplyr::filter(!is.na(hh_id)) %>%
                 dplyr::left_join(meta, by = c("hh_id", "filter_type"))
```

## Calculate weights

* pre-average

```{r calc_pre_wgt}
  grav_pre <- dplyr::select(grav_merged, pre_weigh_1, pre_weigh_2, pre_weigh_3, id) %>%
              tidyr::gather("var", "val", pre_weigh_1, pre_weigh_2, pre_weigh_3) %>%
              dplyr::filter(!is.na(val)) %>%
              dplyr::group_by(id) %>%
              dplyr::summarise(wgt_pre = mean(val))
```

* post-average

```{r calc_post_wgt}
  grav_post <- dplyr::select(grav_merged, post_weigh_1, post_weigh_2, post_weigh_3, id) %>%
               tidyr::gather("var", "val", post_weigh_1, post_weigh_2, post_weigh_3) %>%
               dplyr::filter(!is.na(val)) %>%
               dplyr::group_by(id) %>%
               dplyr::summarise(wgt_post = mean(val))
```

* merge averages

```{r merge_calcs}
  grav_merged <- dplyr::left_join(grav_merged, grav_pre, by = "id") %>%
                 dplyr::left_join(grav_post, by = "id")
```

* mass added

```{r calc_delta_wgt}
  grav_merged <- dplyr::mutate(grav_merged, wgt_delta = wgt_post - wgt_pre)
```

* change in calibration weight

```{r add_cal_wgt}
  grav_merged <- dplyr::mutate(grav_merged, wgt_cal_delta = post_cal_weight - pre_cal_weight)
```

# QC

* Seperate blanks and samples

```{r seperate_types}
  blanks <- dplyr::filter(grav_merged, filter_type == "Blank")

  grav_merged <- dplyr::filter(grav_merged, filter_type == "Uncontrolled Test")
```

* extract notes for gravimetric data

```{r extract_notes}
  notes <- dplyr::filter(notes, grepl("grav|all", notes$inst) == TRUE)
```

* apply flags: `bad` preceeds `maybe` preceeds `good`

```{r initiate_qc_flags}
  test_flags <- dplyr::filter(notes, !grepl("blanks|Blank", notes)) %>%
                dplyr::select(hh_id, qc) %>%
                dplyr::group_by(hh_id) %>%
                dplyr::arrange(qc) %>%
                dplyr::summarise(qc = first(qc))

  blank_flags <- dplyr::filter(notes, grepl("blanks|Blank", notes)) %>%
                 dplyr::select(hh_id, qc) %>%
                 dplyr::group_by(hh_id) %>%
                 dplyr::arrange(qc) %>%
                 dplyr::summarise(qc = first(qc))
```

* merge

```{r add_qc_flag}
  grav_merged <- dplyr::left_join(grav_merged, test_flags, by = "hh_id") %>%
                 dplyr::mutate(id = as.factor(hh_id)) %>%
                 dplyr::mutate(qc = as.factor(ifelse(is.na(qc), "ok", as.character(qc))))

  blanks <- dplyr::left_join(blanks, blank_flags, by = "hh_id") %>%
            dplyr::mutate(id = as.factor(hh_id)) %>%
            dplyr::mutate(qc = as.factor(ifelse(is.na(qc), "ok", as.character(qc))))
```

* mark additional bad tests

```{r add_bad_tests}
  blanks$qc[blanks$hh_id == "IN10"] <- "bad"
  #grav_merged$qc[grav_merged$id == "28B"] <- "bad"
  #grav_merged$qc[grav_merged$id == "29A"] <- "bad"
```

* calibration wgt corrections

```{r calibration_correction}
  grav_merged <- dplyr::mutate(grav_merged, wgt_cal_corr = wgt_delta - wgt_cal_delta)

  blanks <- dplyr::mutate(blanks, wgt_cal_corr = wgt_delta - wgt_cal_delta) 
```

# LOD/LOQ

* calculate the limit of detection and quantification

```{r lod_loq}
  signal <- dplyr::filter(blanks, qc != "bad") %>%
            dplyr::summarise(blank_mean = mean(wgt_cal_corr),
                             lod = sd(wgt_cal_corr) * 3,
                             loq = sd(wgt_cal_corr) * 10)

  blank_mean <- signal$blank_mean[1]
  lod <- signal$lod[1]
  loq <- signal$loq[1]
```

The LOD is `r lod`, the LOQ is `r loq`, and the mean of the blanks is `r blank_mean`.

* subract blank mean and flag the measurements

```{r add_lod_loq_var}
  grav_merged <- dplyr::mutate(grav_merged, mass = wgt_cal_corr - blank_mean) %>%
                 dplyr::mutate(lod = ifelse(mass >= lod, "above", "below"),
                               loq = ifelse(mass >= loq, "above", "below"))
```

# Background analysis

* need to figure out what to do about background

# Tidy and save

* filter bad tests

```{r filter_bad_tests}
  grav_merged <- dplyr::filter(grav_merged, qc != "bad")
```

* add pollutant variable `pol`

```{r add_var}
  grav_merged <- dplyr::mutate(grav_merged, pol = "grav")
```

* select variables 

```{r conv_long}
  grav_merged <- dplyr::select(grav_merged, hh_id, date, pol, mass, avg_flow, dur, lod, loq, qc)
```

```{r save_data}
  saveRDS(grav_merged, file = "../r_files/grav_merged.rds")
```

# Summary

Gravimetric data was collected during `r length(unique(grav_merged$hh_id))` experiments between `r min(grav_merged$date, na.rm = TRUE)` and `r max(grav_merged$date, na.rm = TRUE)`. 

The gravimetric data from test `r setdiff(as.character(samples$hh_id), as.character(grav_merged$hh_id))` was excluded because the filter was damaged.  

# QC plots

## weights

```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=12, fig.height=8}
  ggplot(dplyr::filter(grav_merged, grepl("^[0-9][A-Z]", id)==TRUE),
         aes(id, wgt_delta, color= qc)) +
  geom_point() +
  theme_minimal()
  
  ggplot(dplyr::filter(grav_merged, grepl("^[1][0-9][A-Z]", id)==TRUE),
         aes(id, wgt_delta, color= qc)) +
  geom_point() +
  theme_minimal()
  
  ggplot(dplyr::filter(grav_merged, grepl("^[2-3][0-9][A-Z]", id)==TRUE),
         aes(id, wgt_delta, color= qc)) +
  geom_point() +
  theme_minimal()
  
  ggplot(dplyr::filter(grav_merged, grepl("^[G]", id)==TRUE),
         aes(id, wgt_delta, color= qc)) +
  geom_point() +
  theme_minimal()
```

## calibration

```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=12, fig.height=8}
  ggplot(grav_merged, aes(id, wgt_cal_delta)) +
  geom_point() +
  theme_minimal() +
  ylab("Cal weight delta (mg)") +
  theme(text = element_text(size=8), axis.text.x = element_text(angle=-45, hjust=1))
```

## blanks

```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=12, fig.height=8}
  ggplot(grav_merged, aes(id, wgt_blank_delta)) +
  geom_point() +
  theme_minimal() +
  ylab("Blanks weight delta (mg)") +
  theme(text = element_text(size=8), axis.text.x = element_text(angle=-45, hjust=1)) 
```
