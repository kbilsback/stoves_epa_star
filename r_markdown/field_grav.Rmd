...
```{r load_data}
  field_grav <- readRDS("../r_files/field_grav.rds")
```
...
```{r load_meta}
  ...
  field_filter_meta <- readRDS("../r_files/field_filter_meta.rds")
```
...
```{r }
  grav_merged <- dplyr::filter(field_grav, grepl("^[0-9]|^G[0-9]", id)) %>%
                 dplyr::select(-lod) %>%
...
```{r avg_flows}
 flows <- dplyr::filter(field_flows, inst == "filter_1") %>%
          dplyr::select(-pre_flow_setting, -post_flow_setting, -notes) %>%  # go off bubble meter measures only
          tidyr::gather("var", "val", 3:ncol(.)) %>%
          dplyr::group_by(hh_id) %>%
          dplyr::summarise(avg_flow = mean(val, na.rm = TRUE))
```

* match test times to id

```{r }
 times <- dplyr::select(field_test_times, -date) %>%
