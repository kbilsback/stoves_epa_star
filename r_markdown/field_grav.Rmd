...
```{r avg_flows}
 flows <- dplyr::filter(flows, inst == "filter_1") %>%
          dplyr::select(-notes) %>%  # go off bubble meter measures only
          dplyr::group_by(hh_id) %>%
          dplyr::summarise(avg_flow = mean(val, na.rm = TRUE))
```

* merge gravimetric test data with metadata

```{r meta_merge}
  grav_merged <- dplyr::left_join(grav_merged, grav_meta, by = c("id" = "teflon_id")) %>%
                 dplyr::filter(filter_type == "Uncontrolled Test") %>%
                 dplyr::left_join(flows, by = "hh_id") %>%
                 dplyr::left_join(dplyr::select(test_times, -date), by = "hh_id")
```
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  grav_merged <- dplyr::mutate(grav_merged, wgt_blank_delta = wgt_blank_avg_post - wgt_blank_avg_pre)
```
