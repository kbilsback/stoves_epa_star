...
```{r, echo=FALSE}
  mod <- readRDS("../r_files/temp_fp_mod4.RDS")
...
```{r}
  yhat <- predict(mod, field_temp_p, interval = "predict", re.form = NA)
```
...
```{r}
  Xmat <- model.matrix(~temp*stove, data = field_temp_p)
```
